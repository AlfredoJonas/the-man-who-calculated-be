...
def reach_operation(operation, token, variables={"A": 4, "B": 2}):
    ...
    payload = {"operation_id": operation.id, "variables": json.dumps(variables)}
    return post_api("/api/record", payload, token), variables


def get_operation(
    token, page: int = 1, size: int = 10, filter: str = "", order: str = ""
):
    payload = {"page": page, "size": size, "filter": filter, "order": order}
...
def test_success_operations(
    sample_logged_user_account_token,
    build_sample_operation,
    operation,
    random_string_mock_response,
):
    """
    ...
    response, variables = reach_operation(
        operation, sample_logged_user_account_token.key
    )
    data = response.json()
...
def test_addition_operation_out_of_money(
    sample_logged_user_account_token, sample_addition_record_zero_balance
):
    """
    ...
    assert (
        ...
        == f"User balance({user_balance}) is not enough to perform an operation({operation.type}) of {operation.cost}"
    )


def test_invalid_addition_operation_payload(
    sample_logged_user_account_token, sample_addition_operation
):
    """
    ...
    response, _ = reach_operation(
        operation, sample_logged_user_account_token.key, {"A": 4}
    )
    data = response.json()
    assert response.status_code == 400
    assert (
        data["developer_message"]
        == f"Variables field doesn't have the required fields to proccess the operation {operation.type}"
    )
...
def test_get_filtered_operations_v1(
    sample_logged_user_account_token, sample_operation_records
):
    """
...
def test_get_filtered_operations_v2(
    sample_logged_user_account_token, sample_operation_records
):
    """
...
def test_get_filtered_records_v1(
    sample_logged_user_account_token, build_sample_records
):
    """
...
def test_get_filtered_records_v2(
    sample_logged_user_account_token, build_sample_records
):
    """
    ...
    assert data["total_pages"] == math.ceil(
        len(records[OperationType.SUBSTRACTION.value]) / 10
    )


def test_get_filtered_records_v3(
    sample_logged_user_account_token, build_sample_records
):
    """
...
def test_get_filtered_records_v4(
    sample_logged_user_account_token, build_sample_records
):
...
def test_delete_record(sample_logged_user_account_token, build_sample_records):
    ...
    response = delete_api(
        f"/api/record/delete?id={record.id}", token=sample_logged_user_account_token.key
    )
    data = response.json()
    assert response.status_code == 200
